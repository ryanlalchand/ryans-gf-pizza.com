...
app.get('/OrderNow', (request, response, next) => {
    response.render("order.handlebars");
});

// Step 2: Parse input for order
app.get('/PlaceOrder', (request, response, next) => {
    // Parse the guess field into a number and add to request
    request.guess = parseInt(request.query.guess);
    next();
});
...
app.get('/MakeGuess', (request, response, next) => {
    ...
    if (request.session.isCorrect) {
        response.render("GameOver.handlebars", {guesses: request.session.guesses});
    } else {
        response.render("NextGuess.handlebars", {tooHigh: request.session.tooHigh});
    }
});
